...
<body>
    ...
    <script>
        $(function () {
            //입력값 검증 정규표현식 패턴에 맞는지 검증해주는 //시작은 영어로 시작해서 4-14글자까지..//회원가입 정규표현식 검색해보기..
            ...
            $idInput.on('keyup', e => {//한글자 한글자 쓸 때마다 이벤트걸림//블러이벤트로 하면 다음 칸에 커서 옮길 때 중복확인 이벤트 실행
                ...
                //일치하지 않으면 false를 리턴
                    ...
                    setTimeout(() => {//3초의 딜레이를 줌
                    //아이디 중복확인 비동기 요청//실시간으로 중복확인버튼없이!
                    fetch('/check?type=account&keyword=' + $idInput.val())
                        .then(res => res.text())//텍스트로 response가 하나 오는데
                        .then(flag => {
                            console.log(flag);
                            if (flag === 'true') {
                                //중복된!
